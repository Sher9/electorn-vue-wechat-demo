<template>
  ...
  >
    ...
    <!-- 联系人选择器 -->
    <el-dialog v-model="showContactSelector" title="选择联系人" width="400px" append-to-body>
...
<script setup lang="ts" name="GroupManager">
  ...
  members: [
    {
      ...
      message: '请选择群成员',
      trigger: 'change',
      validator: (_: any, value: Member[], callback: (error?: Error | string) => void) => {
        if (!value || value.length < 2) {
          callback(new Error('至少选择2个群成员'))
        } else {
          callback()
        }
      }
...
const availableContacts = computed(() =>
  contacts.value.filter((contact) => !form.value.members.some((member) => member.id === contact.id))
)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    loading.value = true
    let memberIds = form.value.members.map((m) => m.id)
    memberIds.push(userStore.userId)
    if (isEdit.value) {
      // 编辑群聊
      if (props.groupId) {
        await chatApi.addGroupMembers(props.groupId, memberIds)
      }
    } else {
      // 创建群聊
