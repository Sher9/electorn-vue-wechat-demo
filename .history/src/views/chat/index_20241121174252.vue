    ...
    <div class="chat-list">
      <div class="chat-list-header">
        <div class="header-content">
          ...
          <el-button type="success" :icon="Plus" @click="showGroupManager = true"> </el-button>
        </div>
      </div>
        ...
        >
          <el-badge :value="chat.unreadCount" :hidden="!chat.unreadCount">
            <el-avatar :size="40" :src="chat.avatar">
              {{ chat.isGroup ? chat.groupName?.substring(0, 1) : chat.name?.substring(0, 1) }}
            </el-avatar>
          </el-badge>
          <div class="chat-info">
            <div class="name-row">
...
import { Picture, CirclePlus, Search, Plus, Phone, VideoCamera } from '@element-plus/icons-vue'
...
const uploadImage = async (options: UploadRequestOptions) => {
  try {
    const result = await chatApi.uploadImage(options.file)
...
const loadGroupMembers = async () => {
  ...
  try {
    const data = await chatApi.getGroupMembers(currentChat.value.id)
    groupMembers.value = data.filter((member) => member.id !== userStore.userId)
  } catch (error) {
